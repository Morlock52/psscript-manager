-- PSScript AI and Database Comprehensive Diagnostics
-- =================================================
...
SELECT 
    at.table_name,
    CASE 
        WHEN pst.relname IS NOT NULL THEN 'EXISTS ✅'
        ELSE 'MISSING ❌'
    END as status,
    COALESCE(pst.n_live_tup::text, '0') as row_count,
    COALESCE(pg_size_pretty(pg_relation_size(pst.relid)), 'N/A') as size,
...
FROM ai_tables at
LEFT JOIN pg_stat_user_tables pst ON pst.relname = at.table_name
...
WHERE indexdef LIKE '%vector%' OR indexname LIKE '%embedding%'
...
SELECT 
    ...
    COUNT(*) as count,
    ROUND(100.0 * COUNT(*) / NULLIF((SELECT COUNT(*) FROM scripts), 0), 2) as percentage
FROM scripts s
...
SELECT 
    ...
    COUNT(*) as count,
    ROUND(100.0 * COUNT(*) / NULLIF((SELECT COUNT(*) FROM scripts), 0), 0) as percentage
FROM scripts s
...
WITH recommendations AS (
    ...
    SELECT 
        'MEDIUM' as severity,
        'VACUUM table: ' || relname as recommendation
    FROM pg_stat_user_tables
    ...
    SELECT 
        ...
        ROUND(sum(idx_blks_hit)::float / NULLIF(sum(idx_blks_hit) + sum(idx_blks_read), 0) * 100, 2) || '%' as recommendation
    FROM pg_statio_user_indexes
