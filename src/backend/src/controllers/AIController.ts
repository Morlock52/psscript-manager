...
function getAIMode(): 'mock' | 'real' {
  try {
    ...
    if (process.env.AI_FORCE_REAL === 'true') {
      return 'real';
    }
    if (process.env.AI_MODE) {
      const envMode = process.env.AI_MODE.toLowerCase();
...
class AIController {
  ...
  private async getCached<T>(key: string, ttl: number, resolver: () => Promise<T>): Promise<T> {
  ...
  async generateDocumentation(req: Request, res: Response) {
    try {
      ...
      const cacheKey = `ai:documentation:${scriptId}:${script.updatedAt?.toISOString() ?? 'na'}`;
      const cacheTtl = 1000 * 60 * 60 * 6;
      const documentation = await this.getCached(cacheKey, cacheTtl, async () => {
        if (mode === 'real') {
          const ragResponse = await aiService.rag(
            `Generate documentation for the following PowerShell script. Return JSON with summary, description, parameters, examples, notes, relatedCmdlets, author, lastModified, and version.`,
            script.content,
            5
          );
          ...
          const parsed = extractJsonFromText<any>(ragResponse.data.generated_response);
          if (parsed) {
            return parsed;
          }

          return {
            summary: ragResponse.data.generated_response,
            description: 'AI-generated documentation summary.',
            parameters: {},
            examples: [],
            notes: [],
  ...
  async getRefactoringSuggestions(req: Request, res: Response) {
    try {
      ...
      if (mode === 'real') {
        const ragResponse = await aiService.rag(
          'Provide refactoring suggestions for the script. Respond with JSON array including type, severity, line, original, suggested, explanation, and impact.',
          script.content,
          5
        );
          ...
          suggestions = [
            {
              ...
              original: '',
              suggested: '',
              explanation: ragResponse.data.generated_response,
              impact: 'Review AI-generated guidance'
            }
          ];
        }
      } else {
    ...
    try {
      ...
      if (mode === 'real') {
        const messages: ChatMessage[] = [
          {
            role: 'user',
            content: `Convert this description into a PowerShell script. Respond with JSON containing script, explanation, confidence, and alternatives.\n\nDescription:\n${description}\n\nContext:\n${context || 'None'}`
          }
        ];
        const chatResponse = await aiService.chat(messages, {
          systemPrompt: 'You are a PowerShell automation expert. Always respond with JSON.'
        });
        ...
        const parsed = extractJsonFromText<any>(chatResponse.data.response);
        generatedScript = parsed || {
          script: chatResponse.data.response,
...
if ($processes) {
  ...
  async categorizeWithConfidence(req: Request, res: Response) {
    try {
      ...
      if (mode === 'real') {
        const embeddingResponse = await aiService.createEmbedding(script.content);
        ...
        embedding = await generateEmbedding(script.content);
      }
  ...
  async scanSecurity(req: Request, res: Response) {
    try {
      ...
      const mode = getAIMode();
      const cacheKey = `ai:security-scan:${scriptId}:${script.updatedAt?.toISOString() ?? 'na'}`;
      const cacheTtl = 1000 * 60 * 60;
      const securityScan = await this.getCached(cacheKey, cacheTtl, async () => {
        if (mode === 'real') {
          const analysisResponse = await aiService.analyzeScript(script.content, script.title, 'security');
          if (!analysisResponse.ok) {
            throw new Error(analysisResponse.error);
          }
          const analysis = analysisResponse.data;
          const vulnerabilities = analysis.security_analysis
            ? [
                {
                  type: 'AI-SECURITY',
                  severity: 'medium',
                  title: 'AI Security Analysis',
                  description: analysis.security_analysis,
                  line: null,
                  code: null,
                  remediation: 'Review AI recommendations and apply remediation steps.',
                  cveReferences: [],
                  cvssScore: analysis.security_score || 5
                }
              ]
            : [];
          ...
          return {
            vulnerabilities,
            securityScore: analysis.security_score ?? 5,
            scanDate: new Date(),
            recommendations: [
              'Review AI security analysis findings',
  ...
  async reviewCode(req: Request, res: Response) {
    try {
      ...
      if (mode === 'real') {
        const analysisResponse = await aiService.analyzeScript(script.content, script.title, 'review', {
        ...
        const analysis = analysisResponse.data;
        review = {
          overallScore: analysis.code_quality_score ?? 7,
          categories: {
            security: { score: analysis.security_score ?? 6, issues: analysis.security_analysis ? 1 : 0 },
            performance: { score: 7, issues: 0 },
            readability: { score: analysis.code_quality_score ?? 7, issues: 0 },
            bestPractices: { score: 7, issues: 0 }
          },
          issues: analysis.security_analysis
            ? [
                {
                  category: 'security',
                  severity: 'medium',
                  ...
                  suggestion: 'Review and remediate security findings'
                }
              ]
            : [],
          positives: analysis.optimization || [],
          summary: analysis.purpose
            ? `Review based on AI analysis: ${analysis.purpose}`
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Generate Pester tests for this PowerShell script. Respond with JSON containing framework, version, testSuite, and coverage.\n\n${script.content}`
            }
          ],
          { systemPrompt: 'You are a PowerShell testing expert. Always respond with JSON.' }
        );
        ...
        tests = parsed || {
          framework: 'Pester',
          version: '5.x',
          testSuite: chatResponse.data.response,
          coverage: {
            functions: [],
...
Describe "${script.title} Tests" {
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Explain this PowerShell error and provide causes, solutions, and documentation link. Respond with JSON containing explanation, causes, solutions, and documentation.\n\nError:\n${error}\n\nContext:\n${context || 'None'}`
            }
          ],
          { systemPrompt: 'You are a PowerShell troubleshooting expert. Always respond with JSON.' }
        );
        ...
        return res.json({
          originalError: error,
          explanation: chatResponse.data.response,
          causes: [],
          solutions: [],
          documentation: null,
          additionalHelp: 'Try running with -Verbose or -Debug switches for more information'
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Estimate performance characteristics for this PowerShell script. Respond with JSON containing estimatedExecutionTime, estimatedMemoryUsage, cpuIntensity, diskIO, networkUsage, and recommendations.\n\n${script.content}`
            }
          ],
          { systemPrompt: 'You are a PowerShell performance analyst. Always respond with JSON.' }
        );
        ...
        const parsed = extractJsonFromText<any>(chatResponse.data.response);
        performance = parsed || {
          estimatedExecutionTime: { formatted: 'Unknown' },
          estimatedMemoryUsage: { formatted: 'Unknown' },
          cpuIntensity: 'Unknown',
          diskIO: 'Unknown',
          networkUsage: 'Unknown',
          recommendations: [chatResponse.data.response]
        };
      } else {
  ...
  async mergeScripts(req: Request, res: Response) {
    try {
      ...
      if (mode === 'real') {
        const combinedContent = scripts.map(s => `# ${s!.title}\n${s!.content}`).join('\n\n');
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Merge the following PowerShell scripts using the ${mergeStrategy} strategy. Respond with JSON containing content, title, description, conflicts, and optimizations.\n\n${combinedContent}`
            }
          ],
          { systemPrompt: 'You are a PowerShell automation expert. Always respond with JSON.' }
        );
        ...
        mergedScript = parsed || {
          ...
          title: 'Merged Script',
          description: `Merged ${scripts.length} scripts using ${mergeStrategy} strategy`,
          conflicts: [],
          optimizations: []
        };
      } else {
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Generate a PowerShell script from the "${templateType}" template with the following parameters: ${JSON.stringify(parameters || {})}. Respond with JSON containing title, content, description, customized, and templateType.`
            }
          ],
          { systemPrompt: 'You are a PowerShell automation expert. Always respond with JSON.' }
        );
        ...
        return res.json({
          title: 'Generated Template Script',
          content: chatResponse.data.response,
          description: 'AI-generated template script.',
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Convert the following PowerShell script to ${targetLanguage}. Respond with JSON containing language, extension, converted, notes, conversionQuality, and warnings.\n\n${script.content}`
            }
          ],
          { systemPrompt: 'You are a code conversion expert. Always respond with JSON.' }
        );
        ...
        return res.json({
          ...
          extension: '',
          converted: chatResponse.data.response,
          notes: [],
          conversionQuality: 0.7,
          warnings: []
        });
      }
...
main().catch(console.error);`,
  ...
  async optimizeScript(req: Request, res: Response) {
    try {
      ...
      const mode = getAIMode();
      if (mode === 'real') {
        const analysisResponse = await aiService.analyzeScript(script.content, script.title, 'optimization');
        if (!analysisResponse.ok) {
          throw new Error(analysisResponse.error);
        }
        const analysis = analysisResponse.data;
        return res.json({
          ...
          optimizationLevel,
          optimizations: (analysis.optimization || []).map((item: string) => ({
            type: 'recommendation',
            description: item
          })),
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Analyze dependencies for this PowerShell script. Respond with JSON containing dependencies, compatibility, missingDependencies, and installCommands.\n\n${script.content}`
            }
          ],
          { systemPrompt: 'You are a PowerShell dependency analyst. Always respond with JSON.' }
        );
        ...
        return res.json({
          ...
          compatibility: {
            ...
            powerShellCore: true,
            crossPlatform: false,
            minimumPSVersion: 'Unknown',
            warnings: []
          },
          missingDependencies: [],
          installCommands: [],
          notes: chatResponse.data.response
        });
      }
      ...
      if (mode === 'real') {
        ...
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Generate a PowerShell module named "${moduleName}" (version ${moduleVersion}) from these scripts. Respond with JSON containing name, version, files, installCommand, and publishCommand.\n\n${combinedContent}`
            }
          ],
          { systemPrompt: 'You are a PowerShell module author. Always respond with JSON.' }
        );
  ...
  async chatAssistant(req: Request, res: Response) {
    try {
      ...
      if (mode === 'real') {
        ...
        const chatResponse = await aiService.chat(messages, {
          systemPrompt: 'You are a PowerShell assistant. Provide concise answers and suggestions.',
          sessionId
        });
        ...
        response = {
          reply: chatResponse.data.response,
          suggestions: [],
          sessionId: chatResponse.data.session_id || sessionId,
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Predict execution effects for this PowerShell script with parameters ${JSON.stringify(parameters || {})}. Respond with JSON containing willModifyFiles, willModifyRegistry, willNetworkAccess, willCreateProcesses, requiresElevation, estimatedChanges, potentialRisks, resourceUsage, and executionPlan.\n\n${script.content}`
            }
          ],
          { systemPrompt: 'You are a PowerShell execution safety analyst. Always respond with JSON.' }
        );
        ...
        return res.json({
          ...
          willModifyRegistry: false,
          willNetworkAccess: false,
          willCreateProcesses: false,
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Detect code smells in this PowerShell script. Respond with JSON containing codeSmells, metrics, and recommendations.\n\n${script.content}`
            }
          ],
          { systemPrompt: 'You are a PowerShell code quality expert. Always respond with JSON.' }
        );
      ...
      if (mode === 'real') {
        const chatResponse = await aiService.chat(
          [
            {
              role: 'user',
              content: `Generate a changelog for this PowerShell script from version ${fromVersion || '1.0.0'} to ${toVersion || script.version}. Respond with JSON containing summary, changes, statistics, contributors, and recommendations.\n\n${script.content}`
            }
          ],
          { systemPrompt: 'You are a release note generator. Always respond with JSON.' }
        );
